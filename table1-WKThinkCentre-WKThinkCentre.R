...
## -----------------------------------------------------------------------------
m_table1<-table1(~ sex + age + ulcer + thickness | status, data=melanoma2,
       overall=c(left="Total"), caption=caption, footnote=footnote)
## -----------------------------------------------------------------------------

m_table1_flex <- t1flex(m_table1)
m_table1_flex %>%  save_as_docx(path="m_table1_flex.docx")


library(gtsummary)
gt_table1 <- melanoma2 |> 
  select(sex, age, ulcer, thickness, status) |> 
  tbl_summary(
    by=status,
  ...
  bold_labels() |> 
  add_overall() |> 
  modify_footnote(update = everything() ~ "n / N (%); \n\nᵃ Also known as Breslow thickness") |>
  modify_spanning_header(
    c("stat_1", "stat_2", "stat_3") ~ "**Alive or Death**"
  )
gt_table1 |> 
  as_gt() |> 
  gt::gtsave(filename = "gt_table1.docx") # use extensions .png, .html, .docx, .rtf, .tex, .ltx
## -----------------------------------------------------------------------------
labels <- list(
...
create_summary <- function(x) {
  if (is.factor(x)) {
    table <- table(x)
    return(paste0(table, " (", round(prop.table(table) * 100, 1), "%)"))
  } else {
    return(paste0(round(mean(x), 1), " (", round(sd(x), 1), ")"))
  }
}

# Create summary table
summary_stats <- data.frame(
  Variable = c("Sex", "", "Age (years)", "Ulceration", "", "Thickness (mm)ᵃ"),
  Category = c("Male", "Female", "", "Absent", "Present", ""),
  Alive = c(
    create_summary(melanoma2$sex[melanoma2$status == "Alive"])[1],
    create_summary(melanoma2$sex[melanoma2$status == "Alive"])[2],
    create_summary(melanoma2$age[melanoma2$status == "Alive"]),
    create_summary(melanoma2$ulcer[melanoma2$status == "Alive"])[1],
    create_summary(melanoma2$ulcer[melanoma2$status == "Alive"])[2],
    create_summary(melanoma2$thickness[melanoma2$status == "Alive"])
  ),
  `Melanoma death` = c(
    create_summary(melanoma2$sex[melanoma2$status == "Melanoma death"])[1],
    create_summary(melanoma2$sex[melanoma2$status == "Melanoma death"])[2],
    create_summary(melanoma2$age[melanoma2$status == "Melanoma death"]),
    create_summary(melanoma2$ulcer[melanoma2$status == "Melanoma death"])[1],
    create_summary(melanoma2$ulcer[melanoma2$status == "Melanoma death"])[2],
    create_summary(melanoma2$thickness[melanoma2$status == "Melanoma death"])
  ),
  `Non-melanoma death` = c(
    create_summary(melanoma2$sex[melanoma2$status == "Non-melanoma death"])[1],
    create_summary(melanoma2$sex[melanoma2$status == "Non-melanoma death"])[2],
    create_summary(melanoma2$age[melanoma2$status == "Non-melanoma death"]),
    create_summary(melanoma2$ulcer[melanoma2$status == "Non-melanoma death"])[1],
    create_summary(melanoma2$ulcer[melanoma2$status == "Non-melanoma death"])[2],
    create_summary(melanoma2$thickness[melanoma2$status == "Non-melanoma death"])
  ),
  Total = c(
    create_summary(melanoma2$sex)[1],
    create_summary(melanoma2$sex)[2],
    create_summary(melanoma2$age),
    create_summary(melanoma2$ulcer)[1],
    create_summary(melanoma2$ulcer)[2],
    create_summary(melanoma2$thickness)
  )
)
